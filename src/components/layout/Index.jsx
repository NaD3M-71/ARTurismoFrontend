...
export default function Index() {
  ...
  // Los textos de esta configuración usan el sufijo "_en" (no camelCase)
  const textoInicioBilingue = (campoBase) => {
    if (idioma === 'en') {
      const valorEn = textos[`${campoBase}_en`];
      if (valorEn && String(valorEn).trim()) return valorEn;
    }
    return textos[campoBase] || '';
  };

  const tituloBanner = textoInicioBilingue('titulo_banner');
  const aboutusTitulo = textoInicioBilingue('aboutus_titulo');
  const aboutusCuerpo = textoInicioBilingue('aboutus_cuerpo');
  ...
  return (
    <>
      ...
      <div
        ...
        <h2 className='titulo fw-bold text-white position-relative text-lg'>{tituloBanner}</h2>
        ...
        >
          <input
            ...
            placeholder={t('index.buscarPlaceholder')}
          />
      ...
      <div className='redes amarilloART my-5 py-3 align-content-between'>
        <h3 className='text-center text-dark fw-bold'>{t('index.encontranosRedes')}</h3>
      ...
      <div className='destacados actividadesDestacadas d-flex justify-content-center row m-5'>
          <h3 className="text-dark text-center fw-bold">{t('index.ciudadesDestacadas')}</h3>
      ...
      <div className='actividadesDestacadas destacados d-flex justify-content-center row m-5'>
          <h3 className='text-dark text-center fw-bold'>{t('index.actividadesDestacadas')}</h3>
          <div className=' d-flex justify-content-center'>
            <CarouselSwipper actividades={actividades}></CarouselSwipper>
          </div>
        <a href="/actividades" className='btn btn-celeste vertodas'>{t('index.verTodas')}</a>
      </div>
      <AboutUs titulo={aboutusTitulo} cuerpo={aboutusCuerpo} />
    </>
  );
}
